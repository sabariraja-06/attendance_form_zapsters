...
const LightPillar: React.FC<LightPillarProps> = ({
    ...
    useEffect(() => {
        ...
        if (width === 0 || height === 0) {
            // Wait for layout? Or just return?
            // Let's rely on resize listener to pick it up if it starts at 0
        }
    ...
    `;
        ...
        const handleResize = () => {
            // If we are on mobile, maybe we don't resize the canvas constantly to avoid heavy calcs?
            // But resolution sync is important.
            if (resizeTimeout) {
    ...
    if (!webGLSupported) {
        return (
            <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode }}>
                {/* Silent fallback or simple gradient */}
            </div>
        );
    }
